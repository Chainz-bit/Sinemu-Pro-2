...
@section('page-content')
    <section class="settings-log-page">
        <header class="settings-log-header">
            <h1>Log / Riwayat</h1>
            <p>Riwayat verifikasi dan perubahan status {{ $managerRoleLabelLower }} yang tercatat untuk area super admin.</p>
        </header>
        ...
        <section class="settings-log-summary">
            <article>
                ...
                <small>Seluruh data {{ $managerRoleLabelLower }} yang masuk ke ruang kerja super admin.</small>
            </article>
            <article>
                <span>Menunggu</span>
            ...
            <article>
                ...
                <small>{{ \Illuminate\Support\Str::ucfirst($managerRoleLabelLower) }} yang sudah aktif atau ditolak pada riwayat terbaru.</small>
            </article>
        </section>
        ...
        <section class="report-card settings-log-card">
            <header>
                <form class="settings-log-toolbar" method="GET" action="{{ route('super.settings.history') }}">
                    ...
                    <div class="settings-log-toolbar-right">
                        <input type="text" name="search" class="filter-btn settings-log-search" placeholder="Cari pengelola/instansi/email..." value="{{ $search }}">
            ...
            <div class="settings-log-toolbar-meta">
                <p class="settings-log-toolbar-note">
                    {{ $summary['total'] > 0 ? 'Gunakan filter untuk menelusuri riwayat verifikasi ' . $managerRoleLabelLower . ' dengan lebih cepat.' : 'Riwayat akan muncul setelah ada ' . $managerRoleLabelLower . ' yang masuk dan diproses di area super admin.' }}
                </p>
            </div>
            ...
            <div class="settings-log-list">
                @forelse($histories as $history)
                    ...
                    <article class="settings-log-item {{ $statusKey === 'pending' ? 'is-unread' : 'is-read' }}">
                        <div class="settings-log-item-main">
                            ...
                            <p>
                                {{ $history->instansi ?: 'Instansi belum diisi' }} Ã‚Â·
                                {{ $history->email }}
                            </p>
                            <small>
                ...
                @empty
                    <div class="settings-log-empty">
                        ...
                        <p>Riwayat verifikasi {{ $managerRoleLabelLower }} akan tampil di sini setelah ada aktivitas di area super admin.</p>
                    </div>
                @endforelse
            </div>
