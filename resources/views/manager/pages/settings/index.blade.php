...
@php
    ...
    $pageTitle = 'Pengaturan Sistem - ' . $managerRoleLabel . ' - SiNemu';
    $activeMenu = 'settings';
    $hideSidebar = true;
    $hideSearch = true;
    $topbarBackUrl = manager_route('dashboard');
...
<header class="settings-header">
        ...
        <form method="POST" action="{{ manager_route('settings.update') }}" class="settings-form">
            @csrf
            @method('PUT')
            ...
            <article class="settings-card">
                ...
                <div class="settings-grid">
                    <div class="settings-field">
                        ...
                        <input id="kecamatan" name="kecamatan" type="text" class="form-input" maxlength="100" required value="{{ old('kecamatan', $manager?->kecamatan) }}">
                    ...
                    <div class="settings-field">
                        ...
                        <input id="nama" name="nama" type="text" class="form-input" maxlength="255" required value="{{ old('nama', $manager?->nama) }}">
                        <small class="settings-field-help">Nama penanggung jawab instansi pada halaman {{ $managerRoleLabelLower }}.</small>
                    </div>

                    <div class="settings-field settings-field-full">
                        <label for="email">Email Kontak</label>
                        <input id="email" name="email" type="email" class="form-input" maxlength="255" required value="{{ old('email', $manager?->email) }}">
                    ...
                    <div class="settings-field settings-field-full">
                        ...
                        <textarea id="alamat_lengkap" name="alamat_lengkap" class="form-input settings-textarea" maxlength="1200" required>{{ old('alamat_lengkap', $manager?->alamat_lengkap) }}</textarea>
            ...
            <article class="settings-card">
                ...
                <div class="settings-log-row">
                    ...
                    <a href="{{ manager_route('settings.logs') }}">Lihat Log</a>
                </div>
            </article>

            <div class="settings-actions">
                <span class="settings-dirty-indicator" id="settingsDirtyIndicator" aria-live="polite">Perubahan belum disimpan</span>
                <a href="{{ manager_route('dashboard') }}" class="settings-btn settings-btn-light">Batalkan Perubahan</a>
            ...
            function validateEmailInline() {
                ...
                if (emailInlineError) {
                    emailInlineError.textContent = valid || value.length === 0
                        ? ''
                        : 'Format email tidak valid. Contoh: pengelola@sinemu.id';
                }
