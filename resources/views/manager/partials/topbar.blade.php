    ...
    @if(!($hideTopActions ?? false))
        <div class="top-actions">
            <button type="button" class="notification-trigger" aria-label="Notifikasi" aria-expanded="false" aria-controls="notification-modal">
                ...
                @if(($managerUnreadNotificationsCount ?? 0) > 0)
                    <span class="notification-badge">{{ $managerUnreadNotificationsCount > 9 ? '9+' : $managerUnreadNotificationsCount }}</span>
                @endif
            </button>
            <a href="{{ manager_route('settings') }}" class="top-action-link {{ ($activeMenu ?? '') === 'settings' ? 'active' : '' }}" aria-label="Pengaturan">
                <iconify-icon icon="mdi:cog-outline"></iconify-icon>
            </a>

            <div class="notification-modal" id="notification-modal">
                <div class="notification-head">
                    <strong>Notifikasi {{ $managerRoleLabel }}</strong>
                    <div class="notification-head-actions">
                        <form method="POST" action="{{ manager_route('notifications.read-all') }}">
                            @csrf
                            <button type="submit" class="notification-link-btn">Tandai semua dibaca</button>
                        </form>
                        <form method="POST" action="{{ manager_route('notifications.destroy-all') }}" data-confirm-delete data-confirm-message="Hapus semua notifikasi?">
                            @csrf
                            @method('DELETE')
                ...
                <div class="notification-tabs" aria-label="Ringkasan notifikasi">
                    <span class="notification-tab-info">Belum dibaca <strong>{{ $managerUnreadNotificationsCount ?? 0 }}</strong></span>
                </div>

                <div class="notification-list">
                    @forelse(($managerNotifications ?? collect()) as $notification)
                        <div class="notification-item {{ is_null($notification->read_at) ? 'is-unread' : 'is-read' }}">
                            ...
                            <details class="notification-menu-wrap">
                                ...
                                <div class="notification-menu">
                                    @if(is_null($notification->read_at))
                                        <form method="POST" action="{{ manager_route('notifications.read', $notification->id) }}">
                                            @csrf
                                    ...
                                    <form method="POST" action="{{ manager_route('notifications.destroy', $notification->id) }}" data-confirm-delete data-confirm-message="Hapus notifikasi ini?">
                                        @csrf
                                        @method('DELETE')
