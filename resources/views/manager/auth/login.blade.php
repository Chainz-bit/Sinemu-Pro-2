...
@push('styles')
    @vite('resources/js/entries/manager-auth-login.js')
@endpush
...
@section('content')
    ...
    @endphp
    {{-- Pengelola Barang Login Page Start --}}
    <main class="login-shell login-body">
        {{-- Left Panel Start --}}
        <section class="login-left" aria-label="Informasi portal {{ $managerRoleLabelLower }}">
            <div class="left-content">
                <h1 class="left-title">Kelola laporan barang dengan cepat dan aman</h1>
                <p class="left-subtitle">Portal {{ $managerRoleLabelLower }} SiNemu membantu verifikasi laporan temuan, klaim, dan pemrosesan data secara terstruktur.</p>

                <div class="left-illustration" aria-hidden="true">
                    <img
                        src="{{ asset('img/login-image.png') }}"
                        alt="Ilustrasi Login {{ $managerRoleLabel }}"
        ...
        {{-- Right Panel Start --}}
        <section class="login-right" aria-label="Form login {{ $managerRoleLabelLower }}">
            <div class="right-content">
                ...
                <header class="login-header">
                    <h2>Login {{ $managerRoleLabel }}</h2>
                    <p>Masuk untuk melanjutkan ke dashboard {{ $managerRoleLabelLower }}</p>
                </header>

                <form method="POST" action="{{ manager_route('login') }}" class="login-form" novalidate autocomplete="off">
                    @csrf
                ...
                <p class="register-link">
                    Belum punya akun {{ $managerRoleLabelLower }}?
                    <a href="{{ manager_route('register') }}">Daftar Sekarang</a>
                </p>
            </div>
        </section>
        {{-- Right Panel End --}}
    </main>
    {{-- Pengelola Barang Login Page End --}}
@endsection
